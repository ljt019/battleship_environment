...
class BattleshipEnv(vf.MultiTurnEnv):
    ...
    def env_response(
        ...
        # Check if this is the first call (no emulator state yet)
        ...
        # Generate feedback
        feedback = self._format_game_state(
            move, hit, sunk, game_over, invalid, emulator.get_state()
        )
    ...
    def _format_game_state(
        ...
        for ship in game_state.ships:
            if not ship.is_sunk:
                ...
                elif "cruiser" in ship_name_lower or (
                    ship.size == 3 and remaining_counts["cruiser"] == 0
                ):
                    remaining_counts["cruiser"] = 1
                elif "submarine" in ship_name_lower or (
                    ship.size == 3 and remaining_counts["submarine"] == 0
                ):
